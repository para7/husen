...
<div class={'container'}>
	<img class={'header'} alt="header" src="https://placehold.jp/700x700.png" />

	<div class={'parent'}>
		<div class={'icon'}>
			<UserIcon src="https://placehold.jp/150x150.png" />
		</div>
	</div>
	...
	<div class={'texts'}>
		<div class="username">User Name Hereeeeeeeee</div>
		<div>@{userID}</div>
		<div class={'FollowsLine'}>
...
<style>
	...
	.parent {
		...
		margin-left: 20px; /* Assuming width is always 100, so 100 / 5 = 20px */
	}
	...
	.toolPalette {
		height: 50px; /* Assuming width is always 100, so 100 / 2 = 50px */
		display: flex;
		flex-direction: row;
		gap: 1rem;
