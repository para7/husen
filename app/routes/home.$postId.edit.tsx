...
export const loader = async ({ context, params }: Route.LoaderArgs) => {
	...
	if (state.state !== "authed") {
		return { post: null, tags: [], error: "認証が必要です" };
	}
	...
	if (!post) {
		return { post: null, tags: [], error: "投稿が見つかりません" };
	}
...
export default function EditPost() {
	const { post, tags, error } = useLoaderData<typeof loader>();
	...
	if (error) {
		return (
			<Modal opened={opened} onClose={handleClose} title="エラー" centered>
				<Text>{error}</Text>
				<Button onClick={handleClose} mt="md" fullWidth>
					戻る
				</Button>
			</Modal>
		);
